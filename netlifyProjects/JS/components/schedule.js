...
export function renderSchedule(weekId) {
    ...
    weekDates.forEach((date, index) => {
        ...
        if (dayName === 'שבת') {
            row.innerHTML += '<td colspan="2" class="p-3 bg-blue-50 text-blue-700 font-bold border-r border-l border-slate-300">שבת שלום</td>';
        } else {
            const dayData = scheduleDataForWeek[dayName] || {};
            const morningShiftDetails = dayData.morning || {
                employee: 'none',
                start: DEFAULT_SHIFT_TIMES.morning.start,
                end: DEFAULT_SHIFT_TIMES.morning.end
            };
            const eveningShiftDetails = dayData.evening || {
                employee: 'none',
                start: DEFAULT_SHIFT_TIMES.evening.start,
                end: DEFAULT_SHIFT_TIMES.evening.end
            };

            const eveningCellClass = dayName === 'שישי' ? 'bg-blue-50 text-blue-700 font-bold' : '';
            const eveningCellContent = dayName === 'שישי' ? 'שבת שלום' : createShiftCell(dayName, 'evening', eveningShiftDetails.employee, morningShiftDetails.employee, eveningShiftDetails.start, eveningShiftDetails.end);

            row.innerHTML +=
                `<td class="p-0 border-r border-l border-slate-300">${createShiftCell(dayName, 'morning', morningShiftDetails.employee, eveningShiftDetails.employee, morningShiftDetails.start, morningShiftDetails.end)}</td>` +
                `<td class="p-0 border-r border-l border-slate-300 ${eveningCellClass}">${eveningCellContent}</td>`;
        }
