...
import { createMessage } from '../utils.js'; // Make sure this is imported at the top of the file
...
export async function sendFridaySummaryEmail(startDate, endDate) {
    ...
    updateStatus('מכין סיכום ימי שישי...', 'loading', true);
    ...
    // Iterate through all schedules to find Fridays within the date range
    for (const weekId in allSchedules) {
        const weekData = allSchedules[weekId];
        const weekDates = getWeekDates(new Date(weekId));
        
        const fridayDateObj = weekDates[5]; // Friday is the 6th day (index 5)
        const fridayDateStr = fridayDateObj.toISOString().split('T')[0];

        if (fridayDateStr >= startDate && fridayDateStr <= endDate) {
            const shift = weekData['שישי']?.morning;
            if (shift && (shift.employee === 'מאור' || shift.employee === 'מור')) {
                fridays[shift.employee].push({
                    date: formatDate(fridayDateObj),
    ...
    const recipient = 'maorbens@assuta.co.il'; // Or any other recipient
    const subject = `סיכום משמרות שישי מ-${formatDate(startDate)} עד ${formatDate(endDate)}`;

    // Build the HTML for the email body
    let emailBodyContent = `
        <div style="font-family: 'Rubik', sans-serif; direction: rtl; text-align: right; color: #333;">
            <h2 style="color: #8B5CF6; font-size: 24px; margin-bottom: 20px;">${subject}</h2>
    `;

    ['מאור', 'מור'].forEach(employee => {
        emailBodyContent += `<h3 style="font-size: 18px; color: #333; border-bottom: 2px solid #eee; padding-bottom: 5px; margin-top: 20px;">${employee} (סה"כ: ${fridays[employee].length})</h3>`;
        if (fridays[employee].length > 0) {
            emailBodyContent += `<ul style="list-style-type: none; padding: 0;">`;
            fridays[employee].forEach(shift => {
                emailBodyContent += `<li style="background-color: #f9fafb; margin-bottom: 8px; padding: 10px; border-radius: 5px;">
                    <strong>תאריך:</strong> ${shift.date} | <strong>שעות:</strong> ${shift.start} - ${shift.end}
                </li>`;
            });
            emailBodyContent += `</ul>`;
        } else {
            emailBodyContent += `<p>לא נמצאו משמרות בתקופה זו.</p>`;
        }
    });

    emailBodyContent += `<p style="margin-top: 30px; font-size: 14px; color: #555;">בברכה,<br>מערכת סידור עבודה</p></div>`;
